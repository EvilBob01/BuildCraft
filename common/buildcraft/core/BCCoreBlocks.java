...
public class BCCoreBlocks {
    ...
    public static void preInit() {
        ...
        markerVolume = HELPER.addBlockAndItem(new BlockMarkerVolume(Block.Properties.of(), "block.marker.volume"));
        markerPath = HELPER.addBlockAndItem(new BlockMarkerPath(Block.Properties.of(), "block.marker.path"));
        if (BCLib.DEV) {
            powerTester = HELPER.addBlockAndItem(new BlockPowerConsumerTester(Block.Properties.of(), "block.power_tester"));
        }

        engine = HELPER.addBlockAndItem(new BlockEngine_BC8(Block.Properties.of(), "block.engine.bc"), ItemEngine_BC8::new);
        if (engine != null) {
